...
use bhwi::{bitcoin::psbt::Psbt, common::HostPassphrase};
...
use bhwi_async::signing::ledger::{
    LedgerAddressType, LedgerSigningPlan, extend_account_path_for_policy, ledger_signing_plans,
};
...
async fn display_address(selector: HwiSelector, request: HwiDisplayAddressRequest) -> HwiResponse {
    ...
    let display = match request {
        ...
        HwiDisplayAddressRequest::Descriptor { descriptor } => {
            match singlesig_display_address_from_descriptor(device.device().as_mut(), &descriptor)
                .await
