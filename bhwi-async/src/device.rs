...
use bhwi::common::HostPassphrase;
...
impl Device {
    ...
    pub async fn network_warning(
        &mut self,
        expected: Network,
    ) -> Result<Option<String>, HWIDeviceError> {
        let info = self.info().await?;
        if info.networks.is_empty() || info.networks.contains(&expected) {
            return Ok(None);
        }
        Ok(Some(format!(
            "device {} is on {}, expected {expected}",
            self.name,
            networks_string(&info.networks)
        )))
    }
